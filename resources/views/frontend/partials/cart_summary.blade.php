    ...
    <div class="card-body">
        ...
        <table class="table" style="margin-top: 2rem!important;">
            <tfoot>
                ...
                <!-- Total Amount -->
                <tr class="cart-total">
                    <th class="pl-0 fs-14 text-dark fw-600"><span class="strong-600">{{ translate('Total Amount') }}</span></th>
                    <td class="text-right pr-0 fs-14 fw-600 text-primary">
                        <strong><span>{{ single_price($total) }}</span></strong>
                    </td>
                </tr>
                <!-- Due -->
                <tr class="cart-total" id="advanced_div2" style="display: none;">
                    <th class="pl-0 fs-14 text-dark fw-600 border-top-0"><span class="strong-600">{{ translate('Due') }}</span></th>
                    <td class="text-right pr-0 fs-14 fw-600 text-primary border-top-0">
                        <strong><span>{{ single_price(($total * 0.1) - $total) }}</span></strong>
                    </td>
                </tr>
                <!-- Payable Amount -->
                <tr class="cart-total" id="advanced_div3" style="display: none;">
                    <th class="pl-0 fs-14 text-dark fw-600"><span class="strong-600">{{ translate('Payable Amount') }}</span></th>
                    <td class="text-right pr-0 fs-14 fw-600 text-primary">
                        <strong>৳<span id="total">{{ single_price($total) }}</span></strong>
...
<script>
    ...
    var priceElement2 = document.getElementById("sub_total");
    priceElement.textContent = "{{ $total }}";
    priceElement2.textContent = "{{ $total }}";
  function updatePrice() {
    var paymentMethod = document.querySelector('input[name="payment_option"]:checked').value;
    var price = parseFloat(priceElement.textContent);
    var price2 = parseFloat(priceElement2.textContent);
    ...
    priceElement2.textContent = price2;
    
    
    var shippingAddressDiv2 = document.getElementById("advanced_div2");
    var shippingAddressDiv3 = document.getElementById("advanced_div3");

    if (paymentMethod === "sslcommerz") {
      shippingAddressDiv2.style.display = "none";
      shippingAddressDiv3.style.display = "none";
    } else {
      shippingAddressDiv2.style.display = "";
      shippingAddressDiv3.style.display = "";
    }
  }
</script>